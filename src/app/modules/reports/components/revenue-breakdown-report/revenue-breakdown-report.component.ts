...
import { RevenuBreakingReportChart, RevenuTrackingReportChart } from 'src/app/claim-module-components/models/revenuTrackingCategoryChart';
...
export class RevenueBreakdownReportComponent implements OnInit {
    ...
    allChart = true;
    revenuBreakingReport: RevenuBreakDownReport = new RevenuBreakDownReport();
    ...
    showAllChart() {
        ...
        this.revenuBreakingReport.subcategory = RevenuTrackingReportChart.All;
    }
    showServiceChart(categoryType) {
        this.allChart = false;
        this.serviceOrPayerType = categoryType;
        this.revenuBreakingReport.subcategory = categoryType;
    }
    selectRevenu(event) {
        if (event.value !== '0') {
            const data = this.payersList.find(ele => ele.id === parseInt(this.revenuBreakingReport.payerId));
            this.selectedPayerName = data.name + ' ' + data.arName;
            this.isServiceVisible = true;
            this.revenuBreakingReport.subcategory = RevenuTrackingReportChart.Service;
    ...
    get revenuBreakingEnum() {
        return RevenuBreakingReportChart;
    }
    ...
    generate() {
        const fromDate = moment(this.revenuBreakingReport.fromDate).format('YYYY-MM-DD');
        const toDate = moment(this.revenuBreakingReport.toDate).format('YYYY-MM-DD');
        const obj: RevenuBreakDownReport = {
            payerId: this.revenuBreakingReport.payerId,
            fromDate: fromDate,
            toDate: toDate,
            subcategory: this.revenuBreakingReport.subcategory,
        }

        this.reportSerice.generateRevenuBreakingReport(this.providerId, obj).subscribe(event => {
            if (event.body !== undefined) {
                const data = JSON.parse(event.body);

                this.pieChartLabels = data.labels;
                this.pieChartData = data.data;
                // this.pieChartLabels = ['JAN', 'FEB', 'MAR', 'APR', 'MAY', 'JUN', 'JUL', 'AUG', 'SEP', 'OCT', 'NOV', 'DEC'];
                // this.pieChartData = [
                //     {
