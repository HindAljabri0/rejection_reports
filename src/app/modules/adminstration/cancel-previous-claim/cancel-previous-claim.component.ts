...
export class CancelPreviousClaimComponent implements OnInit {
    ...
    isValidNumberListBundleIdAndClaimsIdentifier() {
        if (this.isCancellByBundleIds && this.CancellClaimForm.controls.requestBundleId.value != null &&
            this.CancellClaimForm.controls.requestBundleId.value.split(',').length > 1000) {
            this.dialogService.showMessage('Error', "Number List of Bundle IDs should be less then 1000", 'alert',
                true, 'OK');
            return false
        ...
        if (!this.isCancellByBundleIds && this.CancellClaimForm.controls.claimIdentifier.value != null &&
            this.CancellClaimForm.controls.claimIdentifier.value.split(',').length > 1000) {
            this.dialogService.showMessage('Error', "Number List of Claim Identifier IDs should be less then 1000",
                'alert',
                true, 'OK');
