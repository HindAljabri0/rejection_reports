...
import { PrescriptionCancelReasonModalComponent } from './prescription-cancel-reason-modal/prescription-cancel-reason-modal.component';
import { PrescriptionProcessedTransactionsComponent } from './prescription-processed-transactions/prescription-processed-transactions.component';
import { PrescriptionCommunicationRequestsComponent } from './prescription-communication-requests/prescription-communication-requests.component';
import { PrescriptionReuseApprovalModalComponent } from './prescription-reuse-approval-modal/prescription-reuse-approval-modal.component';
...
import { PrescriptionTransaction } from 'src/app/models/prescriptionTransactionModel';
...
export class PrescriptionTransactionsComponent implements OnInit {
    ...
    @ViewChild('processedTransactions', { static: false }) processedTransactions: PrescriptionProcessedTransactionsComponent;
    @ViewChild('communicationRequests', { static: false }) communicationRequests: PrescriptionCommunicationRequestsComponent;
    ...
    isSubmitted = false;
    errorMessage: string;
    transactionModel: PaginatedResult<PrescriptionTransaction>;
    transactions = [];
    approvalSearchRequest: PrescriptionApprovalSearchRequest = {}
        ...
        if (this.FormPreAuthTransaction.valid) {
            ...
            this.providerNphiesApprovalService.getPrescriberRequestTransactions(this.sharedServices.providerId, model).subscribe((event: any) => {
                if (event instanceof HttpResponse) {
                    ...
                    this.transactionModel = new PaginatedResult(body, PrescriptionTransaction);
        ...
        action.subscribe((event: any) => {
            if (event instanceof HttpResponse) {
                if (event.status === 200) {
                    ...
                    const dialogConfig = new MatDialogConfig();
                    dialogConfig.panelClass = ['primary-dialog', 'full-screen-dialog', 'view-prescription-details'];
                    dialogConfig.data = {
                        // tslint:disable-next-line:max-line-length
                        detailsModel: body
                    };

                    const dialogRef = this.dialog.open(ViewPrescriptionDetailsComponent, dialogConfig);
                    dialogRef.afterClosed().subscribe(result => {
                        if (result) {
                            if (result.openReUse) {
        ...
        action.subscribe((event: any) => {
            if (event instanceof HttpResponse) {
                if (event.status === 200) {
                    ...
                    const dialogRef = this.dialog.open(PrescriptionReuseApprovalModalComponent, dialogConfig);
                    dialogRef.afterClosed().subscribe(result => {
