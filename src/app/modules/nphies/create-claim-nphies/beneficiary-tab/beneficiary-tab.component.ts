...
@Component({
  ...
  styleUrls: ['./beneficiary-tab.component.css']
})
export class BeneficiaryTabComponent implements OnInit {
  ...
  selectPlan(plan) {
    ...
    if (this.selectedBeneficiary.plans.filter(x => x.payerNphiesId === plan.value)[0]) {
      ...
      this.FormNphiesClaim.controls.insurancePlanPayerId.setValue(parseInt(this.selectedBeneficiary.plans.filter(x => x.payerNphiesId === plan.value)[0].payerId));
      this.FormNphiesClaim.controls.insurancePlanExpiryDate.setValue(this.selectedBeneficiary.plans.filter(x => x.payerNphiesId === plan.value)[0].expiryDate);
      this.FormNphiesClaim.controls.insurancePlanMemberCardId.setValue(this.selectedBeneficiary.plans.filter(x => x.payerNphiesId === plan.value)[0].memberCardId);
      this.FormNphiesClaim.controls.insurancePlanRelationWithSubscriber.setValue(this.selectedBeneficiary.plans.filter(x => x.payerNphiesId === plan.value)[0].relationWithSubscriber);
      this.FormNphiesClaim.controls.insurancePlanCoverageType.setValue(this.selectedBeneficiary.plans.filter(x => x.payerNphiesId === plan.value)[0].coverageType);
