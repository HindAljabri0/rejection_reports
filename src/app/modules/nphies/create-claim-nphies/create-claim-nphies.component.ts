...
export class CreateClaimNphiesComponent implements OnInit {
    ...
    setData(response) {
        ...
        this.Items = response.items.map(x => {
            //console.log("item list = " + JSON.stringify(response.items));
            const model: any = {};
            ...
            if (x.itemDetails && x.itemDetails.length > 0) {
                ...
                x.itemDetails.forEach(y => {
                    ...
                    y.drugSelectionReason = y.pharmacistSelectionReason;
                    y.drugSelectionReasonName = this.sharedDataService.itemMedicationReasonList.filter(e => e.value === x.drugSelectionReason)[0] ? this.sharedDataService.itemMedicationReasonList.filter(e => e.value === x.drugSelectionReason)[0].name : "-";
                    y.prescribedDrugCode = y.prescribedDrugCode;
                    y.pharmacistSelectionReason = y.pharmacistSelectionReason;
                    y.pharmacistSubstitute = y.pharmacistSubstitute;
                    y.reasonPharmacistSubstitute = y.reasonPharmacistSubstitute;
                });
            }
            ...
            model.drugSelectionReason = x.pharmacistSelectionReason;
