...
const _claimReducer = createReducer(
    ...
    on(actions.viewRetrievedClaim, (state, response) => {
        ...
        return ({ ...state, claim: body['claim'], type: type, retrievedClaimProps: props, loading: false, claimBeforeEdit: (state.mode == 'EDIT'? body['claim']:null), claimPropsBeforeEdit: (state.mode == 'EDIT'? props:null) });
    }),
    ...
    on(actions.updateCaseType, (state, { caseType }) => ({ ...state, claim: { ...state.claim, caseInformation: { ...state.claim.caseInformation, caseType: caseType } } })),
