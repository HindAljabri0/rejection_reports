...
export class ClaimValidationService {
  ...
  validateRequires() {
    ...
    const invoiceErrors: FieldError[] = [];
    const genInfoErrors: FieldError[] = [];
    ...
    if (departmentCode == null || departmentCode.trim().length == 0 || Number.isNaN(Number.parseInt(departmentCode, 10))) {
  ...
  _isInvalidPeriod(period: Period) {
    return period == null || ((period.days == null || Number.isNaN(period.days) || period.days <= 0) && (period.months == null
      || Number.isNaN(period.months) || period.months <= 0) && (period.years == null || Number.isNaN(period.years) || period.years <= 0));
  }
}
